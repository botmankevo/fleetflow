...
@router.get("")
def list_expenses(token: dict = verify_token):
    ...
    return airtable.list_records(settings.AIRTABLE_TABLE_EXPENSES, formula=formula)


@router.post("")
def create_expense(payload: ExpenseCreate, token: dict = verify_token):
    carrier_record_id = token.get("carrier_record_id")
    if not carrier_record_id:
        raise HTTPException(status_code=400, detail="Missing carrier_record_id")
    ...
    airtable = AirtableClient()
    return airtable.create_record(settings.AIRTABLE_TABLE_EXPENSES, fields)


@router.patch("/{record_id}")
def update_expense(record_id: str, payload: ExpenseUpdate, token: dict = verify_token):
    airtable = AirtableClient()
    return airtable.update_record(settings.AIRTABLE_TABLE_EXPENSES, record_id, payload.fields)
...
def upload_receipt(record_id: str, file: UploadFile = File(...), token: dict = verify_token):
    ...
    base_path = f"{settings.DROPBOX_ROOT_FOLDER}/{carrier_record_id}/expenses/{record_id}"
