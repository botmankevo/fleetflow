...
async def customer_login(credentials: CustomerLogin, db: Session = Depends(get_db)):
    ...
    # In production, query actual Customer model
    customer_id = 1  # Demo customer ID
    customer_email = credentials.email
    customer_name = credentials.email.split('@')[0].title()
    ...
    session_token = f"customer_{customer_id}_{datetime.utcnow().timestamp()}"
